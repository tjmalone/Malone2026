...
load(['data/model_vB/modelData_DS_PS_cLim-' cellLimit '_vB.mat'])
...
% process outliers
typeData = cell(2,3);
typeDataFilter = cell(2,3);
plotItr = cell(1,3);
for jj = 1:3
    outIdx = cell(1,2);
    for ii = 1:2
        curType = typeNames{ii};
        typeData{ii,jj} = modelData.(useVar).(curType){jj};

        % find outliers
        [~,outIdx{ii}] = rmoutliers(typeData{ii,jj},'ThresholdFactor',5);
    end

    % remove outliers
    outIdxAll = outIdx{1} | outIdx{2};
    for ii = 1:2
        typeDataFilter{ii,jj} = typeData{ii,jj}(~outIdxAll);
    end

    % set random indices
    plotItr{jj} = randsample(sum(~outIdxAll),plotN);
end
...
for ii = 1:2
    ...
    % plot distributions
    curData = typeDataFilter(ii,:);
    for jj = 1:3
        % plot distribution
        refData = curData{jj}(plotItr{jj});
    ...
    statsSumD = 'd = ';
    for kk = 1:nP
        curData1 = curData{pShow(kk,1)};
        curData2 = curData{pShow(kk,2)};

        % subsample data for plotting
        curDataSub1 = curData{pShow(kk,1)}(plotItr{pShow(kk,1)});
        curDataSub2 = curData{pShow(kk,2)}(plotItr{pShow(kk,2)});
        ...
        testCat = [typeNames{ii} ': ' cellTypes{pShow(kk,1)} ' vs. ' cellTypes{pShow(kk,2)}];
...
% save figure
savefig([svFile '/cmpDistribution_PS_cLim-' cellLimit '_vB.fig'])
...
figure
tiledlayout(1,2)
cellTypesCmp = {'all-ste','all-pyr','ste-pyr'};
...
% save figure
savefig([svFile '/powerAnalysis_PS_cLim-' cellLimit '_vB.fig'])
