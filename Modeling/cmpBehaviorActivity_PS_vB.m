...
for cc = 1:length(cellTypes)
    ...
    for ii = 1:nBeh
        ...
        for af = 1:nActFields
            if iscell(mAct)
                ...
                if af==checkFld && length(mActCur)<minCells
                    mActCur = NaN;
                end
            else
                mActCur = mAct;
    ...
    save(['data/model_vB/corrData_' cellTypes{cc} '_PS_vB.mat'],'corrData','corrLabels')
end
...
params = [];
useB = 1;
useA = 4:8;  % all behavior and combined variables
nB = length(useB);
nA = length(useA);
...
svFile = [p1 '/Figures/Figures' char(datetime('today','format','yyyyMMdd')) '/activityCorrPS_vB'];
if ~isfolder(svFile)
    mkdir(svFile);
    mkdir([svFile '/data']);
end
...
for cc = 1:length(cellTypes)
    % load data
    load(['data/model_vB/corrData_' cellTypes{cc} '_PS_vB.mat'])
