...
cellTypes = {'allMorph','pyr','ste'};

% determine downsample numbers
nSamples = 200;

% grid cell indices
cellsP = cellSelect.learn.allSex.common.pyr;
cellsP = combCells(cellsP.WT,cellsP.AD);
NCellsP = cellfun(@length,cellsP);

% non-grid cell indices
cellsS = cellSelect.learn.allSex.common.ste;
cellsS = combCells(cellsS.WT,cellsS.AD);
NCellsS = cellfun(@length,cellsS);
...
nCellsMP = NCellsP(1:2:end,:)+NCellsP(2:2:end,:);
nCellsMS = NCellsS(1:2:end,:)+NCellsS(2:2:end,:);

NSubsampleM = min(nCellsMP,nCellsMS);
...
samplesAll = struct();
samplesAll.P.cellsSmp = cellsSmp{1};
samplesAll.P.nSamples = nSamples;
samplesAll.S.cellsSmp = cellsSmp{2};
samplesAll.S.nSamples = nSamples;
samplesAll.C.cellsSmp = cellsSmp{3};
...
save('/MATLAB Drive/FY2025/Behavior/data/model_vB/activityData_DS_PS_vB.mat','activityData','mouseSubSamples')
