...
% load processed data
load(['data/model_vB/LOOCVplot_' modelType '_' groupSaveName{useGroup} svSuff '_GnG_vB.mat'],...
...
svFile = [p1 '/Figures/Figures' char(datetime('today','format','yyyyMMdd')) '/GnG_vB'];
if ~isfolder(svFile)
    mkdir(svFile);
    mkdir([svFile '/data'])
end
...
for dt = 1:length(dataTypes)
    ...
    if dt==1
        ...
        if strcmp(svSuff,'_grid')
            pieOrder = [1 7 2 6 5 4 3 8 9];
        elseif strcmp(svSuff,'_nongrid')
            pieOrder = [1 3 4 5 7 6 2 8 9];
        else
            error('Invalid input')
        end
    ...
    for mm = 1:nMouse
        ...
        xlabel('Real Behavior')
        ylabel('Model Prediction')
        xlim([0.5 10.5])
        ylim([0 1])

        title([groupIDs{curGroup} '-' sexIDs{curSex+1} ' Mouse ' num2str(mm)...
            ': r = ' num2str(r,2) ', R^2 = ' num2str(R2,2)])
