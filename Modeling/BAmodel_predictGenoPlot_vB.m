...
% load data
svSupp = '_noSG';
load(['data/model_vB/LOOCVplotMin_' svSupp '_vB.mat'],'LOOData')
...
% define cell type sets
typeSetCode = fieldnames(LOOData);
nTypeSets = length(typeSetCode);

for tSet = 1:nTypeSets
    % define cell types
    cellTypes = fieldnames(LOOData.(typeSetCode{tSet}));
    nCellTypes = length(cellTypes);

    % initialize figure
    figure; tiledlayout(1,nCellTypes)
    sgtitle(['WT vs. PS19 Classification: ' typeSetCode{tSet}])

    for cc = 1:nCellTypes
        % get current data
        LOODataCur = LOOData.(typeSetCode{tSet}).(cellTypes{cc});

        % define number of samples
        % initialize classification output
        nSamples = length(LOODataCur.yTest);
        nShuff = size(LOODataCur.yTest{1},2);
        perCorrect = zeros(nShuff,nSex,nSamples);

        % loop through sub samples
        for sample = 1:nSamples
            % define use data
            dataTrue = LOODataCur.yTest{sample};
            dataPred = LOODataCur.yPredTest{sample};
            useMice = LOODataCur.idxMouse{sample};
            nMiceAll = length(useMice);

            % reconstruct mouse table
            tblM = LOODataCur.tblM{sample};
            ...
            for ss = 1:nSex
                ...
                for mm = 1:nMice
                    ...
                    for gg = 1:nGeno
                        ...
                        % don't exclude mice for PS
                        if tSet~=nTypeSets || ss==1
                ...
                perCorrect(:,ss,sample) = mean(curCorrect,1,'omitnan');
            end
        end

        perCorrectMean = mean(perCorrect,3,'omitnan');
        ...
        for ss = 1:nSex
            ...
            plotShuff = perCorrectMean(1:end-1,ss);
            plotTrue = perCorrectMean(end,ss);
