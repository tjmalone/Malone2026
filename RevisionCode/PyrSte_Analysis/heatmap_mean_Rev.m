...
load('Results/Figures_current/timecourse/data/dfofSig_common-cell.mat','mapData')
fullData.dfofPre = mapData.Pre;
fullData.dfofPost = mapData.Post;

% load spatial selectivity heatmap
load('Results/Figures_current/timecourse/data/spatialSelectivity_common-cell.mat','mapData')
fullData.SpatialSelectivity = mapData.All;

% load absolute speed score heatmap
load('Results/Figures_current/timecourse/data/speedScoreAbs_common-cell.mat','mapData')
fullData.SpeedScore = mapData.All;

% load within-day consistency heatmaps
load('Results/Figures_current/timecourse/data/RBR_common-cell.mat','mapData')
fullData.InterLap_Post = mapData.Post;
fullData.InterLap_Diff = mapData.Diff;

% load inter-day consistency heatmaps
load('Results/Figures_current/timecourse/data/inter_common-cell.mat','mapData')
fullData.InterDayAll = mapData.All;
fullData.InterDayPost = mapData.Post;

% load dfof decoding heatmaps
load('Results/Figures_current/timecourse/data/dfofIO_common-cell.mat','mapData')
fullData.dfofIO = mapData.All;

% define deficit directionality (1 = low is good)
mapDir = [-1, 1, -1, -1, -1, -1, -1, -1, 1];

% set save folder name
svFile = [p1 '/Results/Figures' char(datetime('today','format','yyyyMMdd')) '/heatmaps'];
if ~isfolder(svFile)
    mkdir(svFile);
    mkdir([svFile '/data']);
end
...
useData = normData(:,:,useFields);
normMean = mean(useData,3);
...
[~,p] = ttest(useData,0,'Dim',3);

% perform t-test
testNameT = 'two-tailed one-sample Students t-test';
nUnits = 'activity measures';
pair = -1;
MC = 0;
limitP = 0;

outStats = {};
for ii = 1:length(labsMorph)
    for jj = 1:length(labsSex)
        data1 = squeeze(useData(ii,jj,:));
        data2 = zeros(size(data1));
        catCur = [num2str(ii) '-' num2str(jj)];

        outStats(end+1,:) = [catCur ttestEffectSize(data1,data2,testNameT,nUnits,pair,MC,limitP)];
    end
end
