...
% usage: semshading(amatrix,alpha,acolor,F,smth)
...
% remove NaNs from fill
fillY = [amean+astd fliplr(amean-astd)];
fillY(isnan(fillY)) = 0;

if exist('alpha','var')==0 || isempty(alpha) 
    
    fillOut = fill([F fliplr(F)],fillY,acolor,'linestyle','none');
    acolor='k';
else
    fillOut = fill([F fliplr(F)],fillY,acolor, 'FaceAlpha', alpha,'linestyle','none');
end
