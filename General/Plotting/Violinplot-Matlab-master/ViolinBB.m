classdef ViolinBB < handle
    ...
    % Modified by Taylor Malone, 2025
    % Changed defaults. Added x position input (xPos) indpendent of index
    % input pos. Change box plot to vertical line
    ...
    methods
        function obj = ViolinBB(data, pos, xPos, varargin)
            %Violin plots a violin plot of some data at pos
            %   VIOLIN(DATA, POS, xPOS) plots a violin at x-position xPOS for
            %   a vector of DATA points. POS is used as index counter.
            %
            ...
            %                    containing scalar values. Defaults to 1.
            ...
            namefun = st.name;
            args = obj.checkInputs(data, pos, xPos, varargin{:});
            ...
            if numel(data) == 1
                obj.MedianPlot = scatter(xPos, data, 'filled');
            ...
            switch args.DataStyle
                case 'scatter'
                    if ~isempty(data2)
                        jitter = 1*(rand(size(data))); %right
                        obj.ScatterPlot = ...
                            scatter(xPos + jitter.*jitterstrength, data, args.MarkerSize, 'filled');
                        % plot the data points within the violin area
                        ...
                        obj.ScatterPlot2 = ...
                            scatter(xPos + jitter.*jitterstrength, data2, args.MarkerSize, 'filled');         
                    else 
                        obj.ScatterPlot = ...
                            scatter(xPos + jitter.*jitterstrength, data, args.MarkerSize, 'filled');
                ...
                case 'histogram'
                    ...
                    switch args.HalfViolin
                        case 'right'
                            obj.HistogramPlot= plot([xPos-((counts')/max(counts))*max(jitterstrength)*2, xPos*ones(size(counts,2),1)]',...
                                [edges(1:end-1)+max(diff(edges))/2; edges(1:end-1)+max(diff(edges))/2],'-','LineWidth',1, 'Color', 'k');
                        case 'left'
                            obj.HistogramPlot= plot([xPos*ones(size(counts,2),1), xPos+((counts')/max(counts))*max(jitterstrength)*2]',...
                                [edges(1:end-1)+max(diff(edges))/2; edges(1:end-1)+max(diff(edges))/2],'-','LineWidth',1, 'Color', 'k');
                        otherwise
                ...
                switch args.HalfViolin
                    case 'right'
                        obj.ViolinPlot =  ... % plot color will be overwritten later
                            fill([xPos+density*width halfViol*xPos], ...
                            [value value(end:-1:1)], [1 1 1]);
                    case 'left'
                        obj.ViolinPlot =  ... % plot color will be overwritten later
                            fill([halfViol*xPos xPos-density(end:-1:1)*width], ...
                            [value value(end:-1:1)], [1 1 1]);
                    case 'full'
                        obj.ViolinPlot =  ... % plot color will be overwritten later
                            fill([xPos+density*width xPos-density(end:-1:1)*width], ...
                            [value value(end:-1:1)], [1 1 1]);
                end
            else
                % plot right half of the violin
                obj.ViolinPlot =  ...
                    fill([xPos+density*width xPos-density(1)*width], ...
                    [value value(1)], [1 1 1]);
                % plot left half of the violin
                obj.ViolinPlot2 =  ...
                    fill([xPos-densityC(end)*widthC xPos-densityC(end:-1:1)*widthC], ...
                    [valueC(end) valueC(end:-1:1)], [1 1 1]);
            end
                
            %% Plot the quartiles within the violin
            quartiles = quantile(data, [0.25, 0.5, 0.75]);
            flat= [halfViol*xPos halfViol*xPos];
            switch args.QuartileStyle
                case 'shadow'
                    switch args.HalfViolin
                        case 'right'
                            w = [xPos+density*width halfViol*xPos];
                            h= [value value(end:-1:1)];
                        case 'left'
                            w = [halfViol*xPos xPos-density(end:-1:1)*width];
                            h= [value value(end:-1:1)];
                        case 'full'
                            w = [xPos+density*width xPos-density(end:-1:1)*width];
                            h= [value value(end:-1:1)];
                    end
                ...
                case 'boxplot'
                    obj.BoxPlot = ... % plot color will be overwritten later
                        plot([xPos xPos],quartiles([1 3]),'Color',[1 1 1]);
                case 'none'
            end
                
            %% Plot the data mean
            meanValue = mean(data);
            meanDensityWidth=args.Width;
            switch args.HalfViolin
                case 'right'
                    obj.MeanPlot = plot(xPos+[0,1].*meanDensityWidth, ...
                        [meanValue, meanValue]);
                case 'left'
                    obj.MeanPlot = plot(xPos+[-1,0].*meanDensityWidth, ...
                        [meanValue, meanValue]);
                case 'full'
                    obj.MeanPlot = plot(xPos+[-1,1].*meanDensityWidth, ...
                        [meanValue, meanValue]);
            end
            ...
            if ~isempty(lowhisker) && ~isempty(hiwhisker)
                obj.WhiskerPlot = plot([xPos xPos], [lowhisker hiwhisker]);
            end
                
            % Median
            obj.MedianPlot = scatter(xPos, quartiles(2), args.MedianMarkerSize,...
                [1 1 1], 'filled', 'MarkerEdgeColor', 'none');
                
            % Notches
            obj.NotchPlots = ...
                scatter(xPos, quartiles(2)-1.57*IQR/sqrt(length(data)), ...
                [], [1 1 1], 'filled', '^');
            obj.NotchPlots(2) = ...
                scatter(xPos, quartiles(2)+1.57*IQR/sqrt(length(data)), ...
                [], [1 1 1], 'filled', 'v');
        ...
        function set.BoxWidth(obj,width)
            if ~isempty(obj.BoxPlot)
                obj.BoxPlot.LineWidth=width;
            end
        end

        function width = get.BoxWidth(obj)
            width=obj.BoxPlot.LineWidth;
        end
    ...
    methods (Access=private)
        function results = checkInputs(~, data, pos, xPos, varargin)
            ...
            p.addParameter('BoxWidth', 1.0, isscalarnumber);
            p.addParameter('EdgeColor', [0 0 0], iscolor);
            p.addParameter('MedianColor', [1 1 1], iscolor);
            p.addParameter('ViolinAlpha', {1,0.15}, @(x)isnumeric(vertcat(x{:})));
            ...
            p.addParameter('ShowWhiskers', false, isscalarlogical);
            ...
            p.parse(data, pos, xPos, varargin{:});
            results = p.Results;
        end
    end
