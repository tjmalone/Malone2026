function violins = violinplotBB(data, cats, xPos, varargin)
...
% Default values changed. Category names are not sorted.
% Input:
%   xPos - x positions for each violin. Default is integer values.
%
...
if isa(data, 'dataset') || isstruct(data) || istable(data)
    ...
    if ~exist('xPos','var') || isempty(xPos)
        xUse = 1:length(catnames);
    else
        xUse = xPos;
    end
    for n=1:length(catnames)
        thisData = data.(catnames{n});
        violins(n) = ViolinBB({thisData}, n, xUse(n), varargin{:});
    end
...
if not(hascategories) && isvector(data{1})
    violins = ViolinBB(data, 1, 1, varargin{:});
    set(gca, 'XTick', 1);
...
elseif ismatrix(data{1})
    for n=1:size(data{1}, 2)
        ...
        violins(n) = ViolinBB(thisData, n, n, varargin{:});
    end
