function cueCellsAnalysis_sig_LR(~,binWidth)

% calcualte combined left and right cues
...
try
    ...
    cd('cueAnalysisNew_sig')
end

%screen the cells
% (1) at least one in field
%due to baseline problem, some cells do not have out field, which is not
%real.
...
useDfof_sig=dfof_sig(:,useIdx);
useCOMs=COMs(useIdx,:);
useDfofaverage_sig=dfofaveragesmooth_sig(:,useIdx);
save('useDfof_sig.mat','useDfof_sig');
save('useCOMs.mat','useCOMs');
save('useDfofaverage_sig.mat','useDfofaverage_sig');
...
% Modified to specifically analyze left and right cues combined
load('tempRL')
if exist('tempLR','var')
    tempRL = tempLR;
end
...
for ii = 1:numel(folderNames)
    ...
    cueCells.useDfofaverage = useDfofaverage_sig;
    ...
    for n=1:length(useIdx)
        for m=1:nShuffle
            [cueCells.shuffleScores(m,n), ~] = get_cuescoreYGModified_lessLagsNew(useDfofaverage_sig(:,n),cueCells.shuffleTemplates(:,m),lagBins);
        end
    end
    ...
    cueCells.realLags=[];
    for n=1:length(useIdx)
        [cueCells.realScores(1,n), cueCells.realLags(1,n)] = get_cuescoreYGModified_lessLagsNew(useDfofaverage_sig(:,n),temp,lagBins);
    end
