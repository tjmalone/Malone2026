%% alignAuto.m
% Peforms alignments for analysis on a full set of days for one mouse.
%

clear all; close all; clc

warning('off')


%% Identify analysis folders

dataFolder = 'D:\VirusMice';
cd(dataFolder)

saveFolder = 'AlignmentFinal';
imageFile = 'imageFilesNew.mat';
fileType = 'all';

% find all pre-defined folder sets within mouse data
folds = findSubF(imageFile,2,[],0);


%% Run alignment for all comparison sets

for ii = 1:length(folds)
    cd(erase(folds{ii},['\' imageFile]));
    
    mkdir(saveFolder)
    cd(saveFolder)
    
    % check claim
    if exist('claimedA.mat','file')~=0
        disp(0)
        pause(0.1)
        continue
    else
        claimed = 1;
        save('claimedA.mat','claimed')
        disp(1)
    end
    
    % load current comparison set
    load(folds{ii})
    if isstruct(files)
        folders = files.(fileType);
    elseif iscell(files)
        folders = files;
    else
        error('Invalid file type')
    end
    save('folders.mat','folders')
    
    % run alignment
    copyfile('D:\AnalysisCode\Alignment\alignmentSuite2p.m','alignmentSuite2p.m')
    try
        alignmentSuite2p()
    catch
        delete('claimedA.mat');
    end
    
end

cd(dataFolder)
