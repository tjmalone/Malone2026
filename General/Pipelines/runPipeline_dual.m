...
if ~exist([pwd '\runPipeline_dual.m'],'file')
    copyfile([baseFolder 'runPipeline_dual.m'],'runPipeline_dual.m');
end
...
% make Z folders and internal folders/files
for ii = 1:2
    mkdir(outFolders{ii})
    mkdir([outFolders{ii} '\tifStack'])
    mkdir([outFolders{ii} '\suite2p'])
end


%% Set parameters

limLaps = 0;        % remove last incomplete lap (0=off,1>=on)
filter = '*Ch*';   % tiff name filter
NPerStack = 1002;   % number of tiffs per stack
dwnSample = 2;      % downsampling factor
cutPix = 1;
% cutPix = 133;       % first pixel on x axis included in final tiffs
...
save([outFolders{1} '\m.mat'],'m','-v7.3');
...
% split galvo trace
for ii = 1:2
    md = m(middleFrameIdx(ii:2:end),:);
    fRate = mean(diff(md(:,1)))/1000;   % frame rate (sec)
    save([outFolders{ii} '\md.mat'],'md','fRate','dwnSample','-v7.3')
end
...
indivTiffsToStack_dual(folderPath,outFolders,filter,NPerStack,dwnSample,lastIdx,cutPix);
