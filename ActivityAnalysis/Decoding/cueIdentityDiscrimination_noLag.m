%% cueIdentityDiscrimination_noLag
...
for cType = 1:nCellType
    ...
    if isfile(['data\cueIdent\cueIdentityDataNL_' curCellType '.mat']) & newRun==0
        load(['data\cueIdent\cueIdentityDataNL_' curCellType '.mat'])
    else
    ...
    for pt = 1:nPT
        ...
        for ii = 1:nFOV
            ...
            for jj = 1:nDays
                ...
                % calculate activity variation
                if pt==1
                    [curAmpDiffMean,curAmpDifPeak] = ampDiff_TM({useCells},[],cuePairInfo,cuePairs,0);
                else
                    [curAmpDiffMean,curAmpDifPeak] = ampDiff_TM({useCells},[],antiCuePairInfo,antiCuePairs,0);
                end
            ...
            save([p1 '\data\cueIdent\cueIdentityDataNL_' curCellType '.mat'],'cueIdentityData','-v7.3')
        end
    ...
    save(['data\cueIdent\cueIdentityDataCatNL_' curCellType '.mat'],'cueIdentityDataCat','-v7.3')
    ...
    for ss = 1:length(useSexes)
        ...
        for tt = 1:nTypes
                ...
                for pt = 1:nPT
                    ...
                    for stt = 1:length(tGroups{tt})
                        ...
                        if ismember(ff,ffBar) && ismember(pt,ptBar) && ismember(tGroups{tt}(stt),sttBar)
                            ...
                            ylabel(yLabs{ff})
                            set(gca,'fontsize',12)
                            savefig([svFolder '\cueDiscrimNL_' typeNames{tt} '-' fldNames{ff} '_' curCellType '-bar_' curSexID '.fig'])
                        end
                    end
                end
            ...
            savefig([svFolder '\cueDiscrimNL_' typeNames{tt} '_' curCellType '_' curSexID '.fig'])
            ...
            if tt==1
                ...
                savefig([svFolder '\cueDiscrimNL_Tile_' curCellType '_' curSexID])
...
for ss = 1:length(useSexes)
    ...
    for ff = 1:nFields
        ...
        savefig([svFolder '\cueDiscrimNL_allTypes-' fldNames{ff} '_' curSexID '.fig'])

        figure(h2)
        sgtitle([fldNames{ff} ': Pre- versus Post- Learning (all cell types, ' curSexID ' mice)'])
        savefig([svFolder '\cueDiscrimNL_allTypes-' fldNames{ff} '-bar_' curSexID '.fig'])
    end
