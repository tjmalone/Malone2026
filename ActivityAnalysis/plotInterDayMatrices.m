...
interDay = cellfun(@(x) squeeze(x)',dataAllInter(1).meanDfof,'UniformOutput',false);
...
%% Plot example cell activity

useDays = [1 1 1 2];
useCells = [43 510 1013 43];
...
nCells = length(useDays);

figure
tiledlayout(nCells,1)
for ii = 1:nCells
    curData = exampleAct{useDays(ii)}(useCells(ii),:);
