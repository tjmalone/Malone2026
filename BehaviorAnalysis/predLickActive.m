function licking = predLickActive(params,envParams,useLaps,varargin)
%% predLickActive
% Calculates predictive licking from active training in virmen. Calculates
% licks before reward and at other locations, excluding licks immediately
% after reward delivery. Can be set so that only 1 lick is allowed per cm
% (currently all licks are included).
%
% Inputs:
...
for ii = 1:nRuns
    ...
    if isempty(curRew)
        ...
        curZone = [rewStart-preRewBuff,rewStop];
    else
        % if reward recieved, predictive window is one reward zone length
        % before start of reward zone through reward location
        curZone = [rewStart-preRewBuff,curRew];
        rewIdx(ii) = curRewIdx;
    end
    ...
    othLength = trackLength - diff([curZone(1) max([rewStop curRew+postRewBuf])]);
